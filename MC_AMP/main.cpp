                           ...
                           array<float>& endvalues)
{
	...
	const extent<1> tinymt_extent(rank);
	const tinymt_collection<1> rand_collection(tinymt_extent, seed);
	...
	try
	{
		parallel_for_each(endvalues.extent, [=, &endvalues](index<1> idx) restrict(amp)
		{
			...
			tinymt t = rand_collection[idx % 65'536];
...
void warm_up()
{
	...
	calculate_value_at_risk<4>(paths, 10.0f, 0.05f, 0.04f, 300, 10, 0);
	std::cout <<
